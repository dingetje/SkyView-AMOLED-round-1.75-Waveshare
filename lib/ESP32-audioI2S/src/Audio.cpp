...
bool Audio::playChunk() 
{
    ...
    profiler_t profiler(
        ...
        String("()")).c_str());
#endif
//    Serial.println("playChunk: m_validSamples = " + String(m_validSamples) + ", m_curSample = " + String(m_curSample));

    // If we've got data, try and pump it out..
    int16_t sample[2];
    if(getBitsPerSample() == 8) 
    {
        if(getChannels() == 1) // mono mode
        {
            while(m_validSamples) 
            {
        ...
        if(getChannels() == 2) // stereo mode
        {
            while(m_validSamples) 
            {
                ...
                else  // force mono, same data on both channels
                {
                    uint8_t xy = (x + y) / 2;
...
void Audio::processLocalFile() 
{
    ...
    availableBytes = 64 * 1024; // set some large value
...
bool Audio::readID3V1Tag()
{
    ...
    if(m_codec != CODEC_MP3) return false; // MP3 only
